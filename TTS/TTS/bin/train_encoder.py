...
import torch
from torch.utils.data import DataLoadeer
...
def main(args):
    ...
    audio_processor = AudioProcessor(**c.audio)
    encoder_model = setup_encoder_model(c)
    ...
    meta_data_train, meta_data_eval = load_tts_sample(
        c.datasets,
        eval_split=True
    )
    ...
    if c.run_eval :
        eval_data_loader , _, _ = setup_loader(
            audio_processor, 
            is_val = True, 
            verbose = True
        )
    ...
    criterion = encoder_model.get_criterion(c, num_classes)
    ...
    if args.restore_path:
        criterion, args.restore_step = encoder_model.load_checkpoint(
    ...
    n_parameters = count_parameters(encoder_model)

    print("\n > Model has {} parameters".format(n_parameters), flush=True)
