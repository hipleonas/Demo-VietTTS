...
import mutagen
# Thay đổi chiến lược chia sẻ bộ nhớ (shared memory strategy) giữa các process khi dùng
...
def noise_augment_audio(wav):
    ...
    BIT16 = 32768.0
    return wav + (1.0 / BIT16) * np.random.randn(*wav.shape)
...
class TTSDataset(Dataset):
    ...
    def load_wav(self, filename):
        loaded_wav = self.ap.load_wav(filename)
        assert loaded_wav.size > 0
        return loaded_wav
    
    def get_phonemes(self, idx, text):
        dict_phoneme = self.phoneme_dataset[idx]
        assert text == dict_phoneme["text"], f"{text} != {dict_phoneme['text']}"
        assert len(dict_phoneme["token_ids"]) > 0
        return dict_phoneme
    
    def get_f0(self, idx):
        dict_f0 = self.f0_dataset[idx]
        item = self.samples[idx]
        assert item["audio_unique_name"] == dict_f0["audio_unique_name"]
        return dict_f0
...
class PhonemeDataset(Dataset):
    """
    Xây dựng lớp Phoneme Dataset để xử lý dữ liệu âm thanh và chuyển đổi văn bản thành các âm vị (phonemes).
    Các tham số:
    samples: Danh sách các mẫu dữ liệu, mỗi mẫu là một dict chứa thông tin về văn bản, tệp âm thanh, tên người nói, v.v.
        Mỗi sample là 1 list hoặc là 1 dict
    tokenizer: Đối tượng tokenizer để chuyển đổi văn bản thành các âm vị.
    
    cache_path : Đường dẫn 
    ...
    def __getitem__(self, index):
        ...
        ids = self.compute_or_load(string2filename(items["audio_unique_name"]), items["text"], items["language"])
        ph_hat = self.tokenizer.ids_to_text(ids)
        return {"text": items["text"], "ph_hat": ph_hat, "token_ids": ids, "token_ids_len": len(ids)}

    def compute_or_load(self, file_name, text, language):
        """Tính toán hoặc tải các âm vị từ tệp cache."""
        
    
        extension = ".npy"
        cache_path = os.path.join(self.cache_path, file_name + extension)
        try :
            tokens_id = np.load(cache_path)
        except FileNotFoundError:
            tokens_id = self.tokenizer.text_to_ids(text, language=language)
            np.save(cache_path, tokens_id)
        return tokens_id
...
class F0Dataset:
    ...
    Args:
        ...
        (Fundamental Frequency – tần số cơ bản) là một thuộc tính âm học quan trọng dùng trong nhiều mô hình TTS (Text-to-Speech), đặc biệt là các mô hình non-autoregressive như FastSpeech, VITS, Glow-TTS, v.v
    """
